...
public class Employee {
	...
	public static void main(String[] args) {
		...
		// 3. 여성 사원 들의 평균 급여 구하기
		int sumPay = 0; // 여사원들 급여 합계
		count = 0; // 사원수(카운터초기화)
		double avgPay = 0.0; // 여사원 평균 급여
		c = 'F';
		for (int i1 = 0; i1 < gender.length; i1++) {
			if (gender[i1] == c) { // 성별이 여자
				sumPay += pay[i1]; // 급여
				count = count + 1; // 인원수 누적
			}
		}
		avgPay = (double) sumPay / count;
		System.out.printf("여성의 숫자는 :%d명이며 평균 급여는 %.1f 원 입니다", count, avgPay);
		System.out.println();
		...
		// String[] arrPhone = null; // 일본 사원 연착처 배열

		for (int i = 0; i < country.length; i++) {
			if (country[i].equals("일본")) {
				nameString = nameString + name[i] + ","; // 이름연결
				phoneString = phoneString + phone[i] + ","; // 연락처 연결
			}

		}
		// 이름 , 연락처 분리
		String[] arrName = nameString.split(","); // 이름 분리 해서 이름 배열에 할당
		String[] arrPhone = phoneString.split(","); // 연락처 분리해서 연락처 배열에 해당
